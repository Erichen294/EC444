...
static float dt = 0.1; // Time interval (100ms)
...
void control_leds(float control) {
    if (control < 0) {
        gpio_set_level(RED_LED, 1);
        gpio_set_level(GREEN_LED, 0);
        gpio_set_level(BLUE_LED, 0);
    } else if (0 < control && control < 0.5) {
...
void master_init()
{
    ...
    io_conf.pin_bit_mask = (1ULL << RED_LED) | (1ULL << GREEN_LED) | (1ULL << BLUE_LED);
    io_conf.pull_down_en = 0;
    io_conf.pull_up_en = 0;
    gpio_config(&io_conf);

    // Set the initial state of the LEDs (off)
    gpio_set_level(RED_LED, 0);
    gpio_set_level(GREEN_LED, 0);
    gpio_set_level(BLUE_LED, 0);
