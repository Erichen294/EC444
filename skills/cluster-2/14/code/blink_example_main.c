...
// Defines go here
#define BUTTON 14 // GPIO for button
#define ON 1  // High state
#define OFF 0 // Low state
...
int count = 0; // Count should go from 0-15
uint16_t displayBuffer[2]; // Global structure to store the number (2 digits)
uint16_t lookupTable[16][2] = { {0b00111111,0b00111111}, {0b00111111,0b00000110}, {0b00111111,0b01011011}, 
                                {0b00111111,0b01001111}, {0b00111111,0b01100110}, {0b00111111,0b01101101},
                                {0b00111111,0b01111101}, {0b00111111,0b00000111}, {0b00111111,0b01111111},
                                {0b00111111,0b01101111}, {0b00000110,0b00111111}, {0b00000110,0b00000110},
                                {0b00000110,0b01011011}, {0b00000110,0b01001111}, {0b00000110,0b01100110},
                                {0b00000110,0b01101101}}; // Global structure for the segment display for count
int numPresses = 0;

void count_to_display(int num)
{
    // Converts current count to binary for i2c
    displayBuffer[0] = lookupTable[num][0];
    displayBuffer[1] = lookupTable[num][1];
}
...
void app_main(void)
{
    ...
    // Turn off all segment displays
    turn_off_all();

    // Create task to handle timer-based events 
    xTaskCreate(timer_evt_task, "timer_evt_task", 2048, NULL, 5, NULL);

    // Display zero before going into loop
    count_to_display(count);
    display();

    // Polling the state of the button
    while (gpio_get_level(BUTTON) == ON || gpio_get_level(BUTTON) == OFF)
    {
        if (gpio_get_level(BUTTON) == ON)
        {
            numPresses++;
        }
        if (gpio_get_level(BUTTON) == ON && numPresses > 0)
        {
            count = -1;
        }
